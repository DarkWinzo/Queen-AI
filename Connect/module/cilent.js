...
let plaintext = global.session.replaceAll("queen~", "");
let key = 'k!t';
let decryptedPlainText = aes256.decrypt(key, plaintext);
  async function md(){
  let {body} = await got(`https://queen-ai.onrender.com/api/session?id=${decryptedPlainText}`)
  let result = JSON.parse(body).result[0].data;
fs.writeFileSync("./Connect/lib/auth_info_baileys/creds.json" , result);
   }
  md();
*/
